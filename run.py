...
def create_files(parent_folder, list_of_files):
    ...
    for file in list_of_files:
        with file in open((folder / file).resolve(), "a+"):
            file.write(f"this MOCK record written at {str(datetime.datetime.now())}")
...
def do_postprocessing_step(years):
    create_files(f"pilates/postprocessing/output", ["a_postprocess_result.txt"])
    for year in years:
